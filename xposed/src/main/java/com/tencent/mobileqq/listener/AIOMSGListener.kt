...
import com.tencent.mobileqq.listener.data.sub.Message
import com.tencent.mobileqq.listener.data.sub.MessageChainContent
    ...
    override fun onRecvMsg(msgList: ArrayList<MsgRecord>) {
        ...
        if (msg.chatType == MsgConstant.KCHATTYPEGROUP) {
            ...
            GlobalScope.launch {
                ...
                val httpEnabled = sharedPreferences.getBoolean("http", false)
                if(httpEnabled){
                    val httpAddr = sharedPreferences.getString("http_addr",null)
                    if (httpAddr != null) {

                        GlobalClient.post("http://$httpAddr"){
                            ...
                            setBody(
                                Message(
                                    ...
                                    message = ArrayList<Message.MessageChain>().apply {
                                        //添加各种类型的消息
                                        add(Message.MessageChain(Message.MessageChain.MessageChainType.TEXT,
                                            MessageChainContent.Text(text = "114514")
                                        ))
                                        add(Message.MessageChain(Message.MessageChain.MessageChainType.AT,
                                            MessageChainContent.AT(qq = "114514")
                                        ))
                                        add(Message.MessageChain(Message.MessageChain.MessageChainType.IMAGE,
